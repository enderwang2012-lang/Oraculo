...
enum ContextSnapshotBuilder {
    static func snapshot(for date: Date = Date(), calendar: Calendar = .current) -> ContextSnapshot {
        ...
        let weatherCache = WeatherContextCache.load()
        let region = GeoContext.region()
        let altitude = GeoContext.altitudeBand(for: region)
